...
namespace PECOFFBinary.SpecFlow
{
    ...
    public class OptionalHeaderDataDirectoriesSteps
    {
        ...
        public void ThenTheArchitectureShoudBe(string architecture)
        {
            ...
            var optionalHeaderDataDirectories = ScenarioContext.Current.Get<OptionalHeaderDataDirectories>("OptionalHeaderDataDirectories");
            Assert.AreEqual(architectureValue, optionalHeaderDataDirectories.ArchitectureData, string.Format("Assert.AreEqual failed on Architecture.  Expected: <0x{0:X}>.  Actual: <0x{1:X}>", architectureValue, optionalHeaderDataDirectories.ArchitectureData));
        }
